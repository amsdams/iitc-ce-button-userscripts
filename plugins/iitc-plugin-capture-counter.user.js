...
(function () {
    ...
    function wrapper(_plugin_info) {
        ...
        function loadData() {
            ...
            try {
                ...
                if (r)
                    seenGuids = new Set(JSON.parse(r));
            }
            catch (_e) {
                seenGuids = new Set();
            }
    ...
    `;
        ...
        function buildSummary() {
            ...
            ['ENL', 'RES', 'MAC'].forEach(function (tc) {
                ...
                parts.push(`<div class="cc-sum-box cc-sum-${color}${dim}" data-team="${tc}">` +
        ...
        function buildTable() {
            ...
            if (entries.length === 0) {
                ...
                tbody = entries
                    .map(function (e, i) {
                    let cls = '';
                    if (selectedAgent === e.name)
                        cls += ' cc-selected';
                    if (flashAgents.has(e.name))
                        cls += ' cc-new';
        ...
        function openDialog() {
            ...
            const html = `<div id="capture-counter-dialog">` +
        ...
        function bindDialogEvents() {
            ...
            dialog.addEventListener('input', function (e) {
                ...
                if (clearBtn)
                    clearBtn.classList.toggle('visible', filterQuery.length > 0);
            ...
            dialog.addEventListener('click', function (e) {
                ...
                if (sumBox) {
                    ...
                    if (teamFilters.has(team)) {
                        if (teamFilters.size > 1)
                            teamFilters.delete(team);
                    }
                    else {
        ...
        function refreshDialog() {
            ...
            if (so && !document.getElementById('cc-search'))
                so.innerHTML = buildSearchBar();
            const tw = document.getElementById('cc-table-wrap');
            if (tw)
                tw.innerHTML = buildTable();
            const dw = document.getElementById('cc-detail-wrap');
    ...
    // ── Plugin Injection ─────────────────────────────────────────────────────────
    const plugin_info = {};
    if (typeof window.GM_info !== 'undefined' && window.GM_info && window.GM_info.script) {
        plugin_info.script = {
            version: window.GM_info.script.version,
            name: window.GM_info.script.name,
            description: window.GM_info.script.description
        };
    }
    // Inject the wrapper
