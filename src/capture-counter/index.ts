...
function loadData() {
  try {
    ...
    if (g) seenGuids = new Set(JSON.parse(g));
  } catch (_e) {
    captures = {};
    seenGuids = new Set();
  }
...
// ── Helpers ────────────────────────────────────────────────────────────────
const escapeHtml = (s: string) => String(s).replace(/[&<>"']/g, m => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#39;' }[m]!));
const teamClass = (team: string) => team === 'ENLIGHTENED' ? 'ENL' : team === 'RESISTANCE' ? 'RES' : team === 'MACHINA' ? 'MAC' : '';
...
function buildSummary() {
  const t: any = { ENL: 0, RES: 0, MAC: 0 };
  for (const name in captures) {
    const tc = teamClass(captures[name].team);
    if (tc) t[tc] += captures[name].count;
  }
...
function buildTable() {
  let entries = Object.entries(captures).map(([name, info]: [string, any]) => ({ name, tc: teamClass(info.team), count: info.count, lastTs: info.lastTs || 0 }));
...
function openDialog() {
  if (dialogRef) return refreshDialog();
  ...
  dialogRef = (window as any).dialog({ title: `📡 Capture Counter v${header.version}`, html, id: 'capture-counter', closeCallback: () => { if (pollerRef) clearInterval(pollerRef); pollerRef = null; dialogRef = null; } });
  bindDialogEvents();
}
...
function bindDialogEvents() {
  ...
  d.onclick = (e: any) => {
    ...
    if (target.id === 'cc-search-clear') {
      filterQuery = ''; (document.getElementById('cc-search') as any).value = '';
      target.classList.remove('visible');
      const tw = document.getElementById('cc-table-wrap'); if (tw) tw.innerHTML = buildTable();
      ...
      sortMode = target.closest('th').dataset.sort; refreshDialog();
    } else if (target.id === 'cc-btn-reset' && confirm('Reset all data?')) {
      captures = {}; seenGuids.clear(); totalCount = 0; selectedAgent = null; filterQuery = '';
      teamFilters = new Set(['ENL', 'RES', 'MAC']); saveData(); refreshDialog();
    } else if (target.id === 'cc-detail-close') {
      selectedAgent = null; refreshDialog();
    } else if (target.closest('.cc-agent-name')) {
      const name = target.closest('tr').dataset.agent;
      selectedAgent = selectedAgent === name ? null : name; refreshDialog();
    } else if (target.closest('.cc-portal-link')) {
      e.preventDefault(); const p = target.closest('.cc-portal-link').dataset;
      ...
      if (!isNaN(lat)) {
        ...
        if (guid && (window as any).portals?.[guid]) (window as any).selectPortal(guid);
        else if (guid) {
          let att = 0; pollerRef = setInterval(() => {
            if (!dialogRef || ++att > 10 || (window as any).portals?.[guid]) {
              if ((window as any).portals?.[guid]) (window as any).selectPortal(guid);
              clearInterval(pollerRef); pollerRef = null;
            }
          }, 500);
        }
        visitedPortals.add(guid || p.name);
        const dw = document.getElementById('cc-detail-wrap'); if (dw) dw.innerHTML = buildDetailPanel(selectedAgent);
      }
    }
  };
}
...
function refreshDialog() {
  ...
  const countEl = document.querySelector('#capture-counter-dialog .cc-count') as any;
  ...
  if (flashAgents.size) flashTimer = setTimeout(() => { flashAgents.clear(); flashTimer = null; }, 1500);
}
...
function processPlexts(data: any) {
  ...
  result.forEach((entry: any) => {
    ...
    if (!guid || seenGuids.has(guid) || !obj?.plext?.markup || (obj.plext.plextType && obj.plext.plextType !== 'SYSTEM_BROADCAST')) return;
    const markup = obj.plext.markup;
    if (!markup.some((s: any) => s[0] === 'TEXT' && CAPTURE_REGEX.test(s[1].plain))) return;
    const player = markup.find((s: any) => s[0] === 'PLAYER')?.[1];
    if (!player?.plain) return;
    const portal = markup.find((s: any) => s[0] === 'PORTAL')?.[1];
    const name = player.plain, ts = entry[1] || Date.now();
    
    seenGuids.add(guid);
    if (!captures[name]) captures[name] = { count: 0, team: player.team || 'unknown', firstTs: null, lastTs: null };
    const cur = captures[name]; cur.count++; totalCount++;
    if (player.team) cur.team = player.team;
    if (portal?.plain) {
      const latLng = portal.latE6 ? { lat: portal.latE6 / 1e6, lng: portal.lngE6 / 1e6 } : null;
      if (cur.firstTs === null || ts < cur.firstTs) { cur.firstPortal = portal.plain; cur.firstPortalGuid = portal.guid; cur.firstPortalLatLng = latLng; cur.firstTs = ts; }
      if (cur.lastTs === null || ts > cur.lastTs) { cur.lastPortal = portal.plain; cur.lastPortalGuid = portal.guid; cur.lastPortalLatLng = latLng; cur.lastTs = ts; }
    }
    flashAgents.add(name); changed = true;
  });
...
const setup = () => {
  ...
  if (w.IITC?.toolbox?.addButton) w.IITC.toolbox.addButton({ label: '📡 Captures', title: 'Show portal capture leaderboard', action: openDialog });
  else w.$('#toolbox').append(`<a onclick="window.plugin.captureCounter.openDialog();return false;" title="Show portal capture leaderboard">📡 Captures</a>`);
...
if (!w.plugin.captureCounter) w.plugin.captureCounter = {};
